...
async function main() {
  const argv = await yargs(hideBin(process.argv))
    ...
    .option("health-config", {
      type: "string",
      description:
        "Custom path to per-server health thresholds YAML (default: ~/.mcp/health-config.yaml)",
    })
    .help()
    .parseAsync();
  ...
  if (argv.daemon) {
    console.error(
      `[mcp-server-health-monitor] Daemon mode enabled — polling every ${argv.interval}s`,
    );
    let running = true;
    const shutdown = () => {
      running = false;
    ...
    const runCheck = async () => {
      ...
      for (const [serverName, entry] of Object.entries(servers)) {
        ...
        if (cmd.includes("mcp-server-health-monitor")) continue;
        try {
          ...
          console.error(
            `[mcp-server-health-monitor] ${serverName}: ${status} (${result.latencyMs ?? "N/A"}ms)`,
          );
        } catch (err) {
