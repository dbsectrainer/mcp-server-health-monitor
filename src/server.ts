...
import type { McpServerEntry, ServerHealthStatus } from "./types.js";
...
function loadManualServers(filePath: string): Record<string, Omit<McpServerEntry, "name">> {
  try {
    if (fs.existsSync(filePath)) {
      const raw = fs.readFileSync(filePath, "utf-8");
      return JSON.parse(raw) as Record<string, Omit<McpServerEntry, "name">>;
    }
  } catch {
...
function saveManualServers(
  filePath: string,
  servers: Record<string, Omit<McpServerEntry, "name">>,
): void {
  fs.writeFileSync(filePath, JSON.stringify(servers, null, 2), "utf-8");
}

function mergeManualServers(
  discovered: Record<string, McpServerEntry>,
  manualServersPath: string | undefined,
): Record<string, McpServerEntry> {
  if (!manualServersPath) return discovered;
 ...
 */
function isSelf(server: McpServerEntry): boolean {
  ...
  if (cmd.includes("mcp-server-health-monitor")) return true;

  // Check if npx invocation targets us
  if (cmd === "npx" || cmd === "npx.cmd") {
    const targetArg = args.find((a) => a.includes("mcp-server-health-monitor"));
    if (targetArg) return true;
  }

  // Compare against current process argv
  const currentScript = argv[1] ?? "";
  if (currentScript && args.some((a) => a === currentScript)) return true;

  return false;
}

function hashSchema(toolsJson: string): string {
  return crypto.createHash("sha256").update(toolsJson).digest("hex").slice(0, 16);
}

function generateSparkline(history: { latency_ms: number | null; status: string }[]): string {
...
function generateDashboardHtml(
  statuses: {
    ...
    history: { latency_ms: number | null; status: string }[];
  }[],
  generatedAt: Date,
): string {
  ...
  const formatDate = (ts: number | null): string => (ts ? new Date(ts).toLocaleString() : "Never");
...
export function createHealthMonitorServer(options: HealthMonitorOptions): Server {
  ...
  let healthConfig: HealthConfig = loadHealthConfig(healthConfigPath);

  // Cancellation flag for health_check_all
  let cancelHealthCheck = false;

  const server = new Server(
    { name: "mcp-server-health-monitor", version: "0.2.0" },
      ...
      tools: [
        {
          ...
          description:
            "Ping every configured MCP server via list_tools, measure latency, and store results. Returns a summary of all servers.",
          ...
          inputSchema: {
            ...
            properties: {
              timeout_ms: {
                type: "number",
                description:
                  "Per-server probe timeout in milliseconds (default: 5000). Example: 5000",
              },
            },
          ...
          inputSchema: {
            ...
            properties: {
              server_name: {
                type: "string",
                description:
                  "The name of the MCP server to query. Example: 'filesystem' (use the key from your MCP config)",
              },
            },
          ...
          inputSchema: {
            ...
            properties: {
              latency_threshold: {
                type: "number",
                description:
                  "Override latency threshold in ms (default: configured threshold). Example: 2000 (flag servers slower than 2 seconds)",
              },
            },
            additionalProperties: false,
          },
        },
        {
          name: "check_updates",
          description:
            "Compare current tool schemas against last stored schemas to detect version drift across MCP servers.",
          annotations: { readOnlyHint: true },
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Generate a self-contained HTML health dashboard with server status, latency trends, and uptime sparklines.",
          annotations: { readOnlyHint: true },
          inputSchema: {
            type: "object",
            properties: {
              output_path: {
                type: "string",
                description:
                  "Optional file path to write the HTML output. Example: '~/Desktop/mcp-health.html'. If omitted, HTML is returned as text.",
              },
            },
            additionalProperties: false,
          },
        },
        {
          name: "get_history",
          description:
            "Return the raw health check history for a specific MCP server, ordered most-recent first.",
          annotations: { readOnlyHint: true },
          inputSchema: {
            type: "object",
            properties: {
        ...
        },
        {
          ...
          description:
            "Register a new MCP server to monitor. Servers added this way are stored in a supplementary config file alongside the database and are merged with servers discovered from Claude's config. Use this to monitor servers not present in Claude Desktop's config.",
        ...
        },
        {
          name: "remove_server",
          description:
            "Remove a manually configured MCP server from monitoring. Only affects servers added via configure_server — servers discovered from Claude's config are not affected.",
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    if (name === "health_check_all") {
      ...
      const timeoutMs = (args as { timeout_ms?: number } | undefined)?.timeout_ms ?? 5000;
      ...
      for (let i = 0; i < filteredServers.length; i++) {
        ...
        try {
          ...
          let status: ServerHealthStatus = result.status;
          if (
            status === "healthy" &&
          ...
          probeEntry = {
            name: serverName,
            status: "offline" as ServerHealthStatus,
            latency_ms: null,
            tool_count: null,
          };
        }
    ...
    if (name === "get_server_status") {
      const typedArgs = args as { server_name: string };
      ...
      if (!(serverName in knownServers)) {
        throw new McpError(
          ...
          `Unknown server: "${serverName}". Run health_check_all to discover servers.`,
        );
      }
      ...
      const serverFirstSeen = db
        .prepare<
          [string],
          { first_seen: number }
        >(`SELECT MIN(checked_at) as first_seen FROM health_checks WHERE server_name = ?`)
        .get(serverName);
    ...
    if (name === "list_degraded") {
      const typedArgs = args as { latency_threshold?: number } | undefined;
      ...
      const degraded = serverNames
        ...
        .filter((s) => {
          const perServerThreshold = getServerThresholds(
            healthConfig,
            s.name,
            threshold,
          ).latency_threshold_ms;
          if (s.status === "offline" || s.status === "degraded") return true;
          if (s.latency_ms !== null && s.latency_ms > perServerThreshold) return true;
          return false;
        });
    ...
    if (name === "check_updates") {
      ...
      const results = await Promise.allSettled(
        filteredServers.map(async ([serverName, entry]) => {
          try {
    ...
    if (name === "export_dashboard") {
      const typedArgs = args as { output_path?: string } | undefined;
    ...
    // ------------------------------------------------------------------
    if (name === "get_history") {
      const typedArgs = args as { server_name: string; limit?: number };
      const serverName = typedArgs?.server_name;
      if (!serverName || typeof serverName !== "string") {
        throw new McpError(ErrorCode.InvalidParams, "server_name is required and must be a string");
      }
    ...
    if (name === "configure_server") {
      if (!manualServersPath) {
        throw new McpError(
          ...
          "Manual server registry is not configured. Restart the server with a database path to enable this feature.",
        );
      }
    ...
    if (name === "remove_server") {
      ...
      if (!(typedArgs.name in manual)) {
        ...
        if (typedArgs.name in discovered) {
          throw new McpError(
            ErrorCode.InvalidParams,
            `"${typedArgs.name}" is a discovered server (from Claude's config) and cannot be removed via this tool.`,
          );
        }
        throw new McpError(
          ErrorCode.InvalidParams,
          `Server "${typedArgs.name}" was not found in the manual registry.`,
        );
      }
...
export async function startServer(options: HealthMonitorOptions): Promise<void> {
